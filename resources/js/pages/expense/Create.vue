...
import type { ExpenseFormData } from '@/types/domains';
import { ExpenseCategory, getValidationErrors } from '@/types/domains';
...
const submit = async () => {
    ...
    try {
        await expenseService.create(formData.value);
...
<template>
    ...
    <div class="space-y-6 p-4 md:p-6">
        <section class="flex flex-col gap-4 rounded-3xl border border-sidebar-border/70 bg-background p-6 shadow-sm lg:flex-row lg:items-end lg:justify-between">
            <div class="space-y-3">
                <p class="text-xs uppercase tracking-[0.3em] text-amber-700">New expense</p>
                <h1 class="text-3xl font-semibold tracking-tight">Record expense</h1>
            </div>
                ...
                <form @submit.prevent="submit" class="space-y-6">
                    <div class="grid gap-6 md:grid-cols-2">
                        <div class="space-y-2">
                            ...
                            <p v-if="errors.property_id" class="text-sm text-red-500">{{ errors.property_id }}</p>
                        </div>
                        <div class="space-y-2">
