...
import type { MeterReading, MeterReadingFormData } from '@/types/domains';
import { getValidationErrors } from '@/types/domains';

const props = defineProps<{ meterReading: MeterReading }>();
const formData = ref<MeterReadingFormData>({
    property_id: props.meterReading.property_id,
    unit_id: props.meterReading.unit_id,
    reading_type: props.meterReading.reading_type,
    previous_reading: props.meterReading.previous_reading,
    current_reading: props.meterReading.current_reading,
    recorded_at: props.meterReading.recorded_at,
});
...
const submit = async () => {
    ...
    try {
        await meterReadingService.update(props.meterReading.id, formData.value);
...
<template>
    ...
    <div class="space-y-6 p-4 md:p-6">
        <section class="flex flex-col gap-4 rounded-3xl border border-sidebar-border/70 bg-background p-6 shadow-sm lg:flex-row lg:items-end lg:justify-between">
            <div><h1 class="text-3xl font-semibold">Reading</h1></div>
