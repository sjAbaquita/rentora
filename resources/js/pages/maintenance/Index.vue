...
import { index, create, edit } from '@/routes/maintenance';
import { maintenanceService } from '@/Services/MaintenanceService';
import type { Maintenance } from '@/types/domains';
...
const isEmpty = computed(() => props.maintenances.length === 0);
const count = computed(() => props.maintenances.length);
...
return;
    ...
    try {
        ...
        error.value = 'Failed to delete maintenance request. Please try again.';
        console.error(err);
    } finally {
...
<template>
    ...
    <div class="space-y-6 p-4 md:p-6">
        <section class="flex flex-col gap-4 rounded-3xl border border-sidebar-border/70 bg-background p-6 shadow-sm lg:flex-row lg:items-end lg:justify-between">
            <div class="space-y-3">
                <p class="text-xs uppercase tracking-[0.3em] text-emerald-700">Maintenance Management</p>
                <div class="space-y-1">
                    <h1 class="text-3xl font-semibold tracking-tight">Maintenance Requests</h1>
                    <p class="max-w-2xl text-sm text-muted-foreground">
                        Manage maintenance requests. Create, view, edit, or delete maintenance records.
                    </p>
                </div>
            </div>
        ...
        <div v-if="isEmpty" class="rounded-lg border-2 border-dashed border-sidebar-border/70 bg-muted/20 p-12 text-center">
            <div class="space-y-4">
                <h3 class="font-semibold text-foreground">No maintenance requests yet</h3>
                <Link
                    :href="create().url"
                    class="inline-flex items-center gap-2 rounded-md bg-emerald-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-emerald-700"
                >
        ...
        <div v-if="!isEmpty" class="overflow-hidden rounded-lg border border-sidebar-border/70">
            <table class="w-full">
                ...
                <tbody class="divide-y divide-sidebar-border/70">
                    <tr v-for="maintenance in maintenances" :key="maintenance.id" class="hover:bg-muted/30 transition">
                        <td class="px-6 py-4">
                            <p class="font-medium text-foreground">{{ maintenance.unit_id }}</p>
                        </td>
                        <td class="px-6 py-4">
                            <p class="text-sm text-muted-foreground">{{ maintenance.priority }}</p>
                        </td>
                        <td class="px-6 py-4">
                            <Badge variant="outline" class="capitalize">{{ maintenance.status }}</Badge>
                        </td>
                        <td class="px-6 py-4">
                            <p class="text-sm text-muted-foreground">{{ maintenance.created_at }}</p>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <button
                                    @click="handleEdit(maintenance.id)"
                                ...
                                >
                                    <Loader v-if="deletingId === maintenance.id" class="h-3.5 w-3.5 animate-spin" />
