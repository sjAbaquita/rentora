...
import type { Lease, LeaseFormData } from '@/types/domains';
...
const formData = ref<LeaseFormData>({
    ...
    lease_start: props.lease.lease_start,
    lease_end: props.lease.lease_end,
...
const submit = async () => {
    ...
    try {
        await leaseService.update(props.lease.id, formData.value);
        router.visit('/leases');
    } catch (err) {
...
<template>
    ...
    <div class="space-y-6 p-4 md:p-6">
        <section class="flex flex-col gap-4 rounded-3xl border border-sidebar-border/70 bg-background p-6 shadow-sm lg:flex-row lg:items-end lg:justify-between">
            <div class="space-y-3">
                <p class="text-xs uppercase tracking-[0.3em] text-amber-700">Edit lease</p>
                <div class="space-y-1">
                    <h1 class="text-3xl font-semibold tracking-tight">Lease #{{ lease.id }}</h1>
