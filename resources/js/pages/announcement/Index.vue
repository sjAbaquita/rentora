...
import { index, create, edit } from '@/routes/announcements';
import { announcementService } from '@/Services/AnnouncementService';
import type { Announcement } from '@/types/domains';
...
const isEmpty = computed(() => props.announcements.length === 0);
const count = computed(() => props.announcements.length);
...
return;
    ...
    try {
        ...
        error.value = 'Failed to delete announcement. Please try again.';
        console.error(err);
    } finally {
...
<template>
    ...
    <div class="space-y-6 p-4 md:p-6">
        <section class="flex flex-col gap-4 rounded-3xl border border-sidebar-border/70 bg-background p-6 shadow-sm lg:flex-row lg:items-end lg:justify-between">
            <div class="space-y-3">
                <p class="text-xs uppercase tracking-[0.3em] text-emerald-700">Announcement Management</p>
                <div class="space-y-1">
                    <h1 class="text-3xl font-semibold tracking-tight">Announcements</h1>
                    <p class="max-w-2xl text-sm text-muted-foreground">
                        Manage tenant announcements. Create, view, edit, or delete announcements.
                    </p>
                </div>
            </div>
        ...
        <div v-if="isEmpty" class="rounded-lg border-2 border-dashed border-sidebar-border/70 bg-muted/20 p-12 text-center">
            <div class="space-y-4">
                <h3 class="font-semibold text-foreground">No announcements yet</h3>
                <Link
                    :href="create().url"
                    class="inline-flex items-center gap-2 rounded-md bg-emerald-600 px-4 py-2 text-sm font-medium text-white transition hover:bg-emerald-700"
                >
            ...
            <table class="w-full">
                <thead class="border-b border-sidebar-border/70 bg-muted/50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-sm font-semibold">Publish At</th>
                        <th class="px-6 py-3 text-center text-sm font-semibold">Actions</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-sidebar-border/70">
                    <tr v-for="announcement in announcements" :key="announcement.id" class="hover:bg-muted/30 transition">
                        <td class="px-6 py-4">
                            <p class="font-medium text-foreground">{{ announcement.title }}</p>
                        </td>
                        <td class="px-6 py-4">
                            <p class="text-sm text-muted-foreground">{{ announcement.priority }}</p>
                        </td>
                        <td class="px-6 py-4">
                            <p class="text-sm text-muted-foreground">{{ announcement.publish_at }}</p>
                        </td>
                        <td class="px-6 py-4">
                            <div class="flex items-center justify-center gap-2">
                                <button
                                    @click="handleEdit(announcement.id)"
                                ...
                                >
                                    <Loader v-if="deletingId === announcement.id" class="h-3.5 w-3.5 animate-spin" />
