...
import type { Announcement, AnnouncementFormData } from '@/types/domains';
...
const formData = ref<AnnouncementFormData>({
    property_id: props.announcement.property_id,
    title: props.announcement.title,
    content: props.announcement.content,
    priority: props.announcement.priority,
    status: props.announcement.status,
    publish_at: props.announcement.publish_at ?? undefined,
    audience: props.announcement.audience ?? undefined,
});
...
const submit = async () => {
    ...
    try {
        await announcementService.update(props.announcement.id, formData.value);
...
<template>
    ...
    <div class="space-y-6 p-4 md:p-6">
        <section class="flex flex-col gap-4 rounded-3xl border border-sidebar-border/70 bg-background p-6 shadow-sm lg:flex-row lg:items-end lg:justify-between">
            <div class="space-y-3">
                <p class="text-xs uppercase tracking-[0.3em] text-amber-700">Edit</p>
                <h1 class="text-3xl font-semibold tracking-tight">{{ announcement.title }}</h1>
            </div>
