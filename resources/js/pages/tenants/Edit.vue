...
import type { Tenant, TenantFormData } from '@/types/domains';
...
const formData = ref<TenantFormData>({
    ...
    date_of_birth: props.tenant.date_of_birth ?? undefined,
...
const submit = async () => {
    ...
    try {
        await tenantService.update(props.tenant.id, formData.value);
...
<template>
    ...
    <div class="space-y-6 p-4 md:p-6">
        <section class="flex flex-col gap-4 rounded-3xl border border-sidebar-border/70 bg-background p-6 shadow-sm lg:flex-row lg:items-end lg:justify-between">
            <div class="space-y-3">
                <p class="text-xs uppercase tracking-[0.3em] text-amber-700">Edit tenant</p>
                <div class="space-y-1">
                    <h1 class="text-3xl font-semibold tracking-tight">{{ tenant.first_name }} {{ tenant.last_name }}</h1>
