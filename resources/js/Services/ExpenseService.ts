/**
 * Expense Service
 * Handles mutations (create, update, delete) for expense management
...
import { store, update, destroy } from '@/routes/expenses';
import type { ExpenseFormData } from '@/types/domains';

class ExpenseService {
    async create(formData: ExpenseFormData): Promise<void> {
        await router.post(store.url(), formData as Record<string, FormDataConvertible>);
    }

    async update(id: number, formData: ExpenseFormData): Promise<void> {
