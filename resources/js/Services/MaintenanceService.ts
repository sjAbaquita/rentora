/**
 * Maintenance Service
 * Handles mutations (create, update, delete) for maintenance management
...
import { store, update, destroy } from '@/routes/maintenance';
import type { MaintenanceFormData } from '@/types/domains';

class MaintenanceService {
    async create(formData: MaintenanceFormData): Promise<void> {
        await router.post(store.url(), formData as Record<string, FormDataConvertible>);
    }

    async update(id: number, formData: MaintenanceFormData): Promise<void> {
