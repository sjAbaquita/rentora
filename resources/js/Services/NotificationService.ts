 ...
 * Handles mutations (create, update, delete) for notification management
...
import { store, update, destroy } from '@/routes/notifications';
import type { NotificationFormData } from '@/types/domains';

class NotificationService {
    async create(formData: NotificationFormData): Promise<void> {
        await router.post(store.url(), formData as Record<string, FormDataConvertible>);
    }

    async update(id: number, formData: NotificationFormData): Promise<void> {
