/**
 * Tenant Service
 * Handles mutations (create, update, delete) for tenant management
...
import { store, update, destroy } from '@/routes/tenants';
import type { TenantFormData } from '@/types/domains';

class TenantService {
    async create(formData: TenantFormData): Promise<void> {
        await router.post(store.url(), formData as Record<string, FormDataConvertible>);
    }

    async update(id: number, formData: TenantFormData): Promise<void> {
